...
from environments.SumoEnvMulti_md_action_mask import SumoEnvMulti
from networks.ctde_mask_r import CTDEActionMask
...
def train_ctde():
    ...
    log_dir = '/home/ytj/PycharmProjects/MARL_TSC/logs/r_bus/peak/ctde/mask_md'

    env = env_creator({})
    register_env('sumo_env', env_creator)
    # Register policy
    ModelCatalog.register_custom_model('CustomModel', CTDEActionMask)
    ...
    config = (
        ...
        .training(
            ...
            gamma=0.65,
            # gamma=tune.grid_search([0.65, 0.7, 0.75, 0.8, 0.85, 0.9]),
            # lr_schedule=[[0, 0.001], [1e6, 0.0005]],
            lr=0.001,
            ...
            vf_loss_coeff=0.5,
            # vf_loss_coeff=tune.grid_search([0.5, 0.1, 0.05, 0.01]),
            entropy_coeff=0.01,
            # entropy_coeff_schedule=[[0, 0.01], [1e6, 0.001]],
            # entropy_coeff=tune.grid_search([0.1, 0.05]),
            clip_param=0.2,
            # clip_param=tune.grid_search([0.2, 0.3, 0.4]),
            grad_clip=0.5,
        )
...
if __name__ == "__main__":
    # train_ctde()
    # policy_path = 'logs/r_bus/offpeak/ctde/mask_md/PPO_2024-03-13_13-23-48/PPO_sumo_env_755ea_00006_6_vf_loss_coeff=0.0500_2024-03-13_13-23-50/checkpoint_000092'
    # test_ctde(policy_path)
    # evaluate_ctde(policy_path)
    evaluation('logs/r_bus/peak/ctde/mask_md/PPO_2024-03-28_15-05-30')
