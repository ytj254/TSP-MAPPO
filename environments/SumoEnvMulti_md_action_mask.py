...
This environment is for multi-discrete action space with action mask.
...
def action_mask(agent):
    valid_actions = np.ones(num_phases+40, dtype=np.int64)
    if agent == 'h2':
        valid_actions[3:8] = 0
...
class SumoEnvMulti(ParallelEnv):
            ...
            observation_space = spaces.Dict(
                {
                    ...
                    'action_mask': spaces.Box(0, 1, shape=(num_phases+40,), dtype=np.int64)
                }
            )
    ...
    def action_space(self, agent):
        return spaces.MultiDiscrete([num_phases, 40])  # The action spaces are identical for parameter sharing
            ...
            # Agent should act now
                ...
                if agent in actions.keys():
                    ...
                    if new_phase_state != current_state:
                        ...
                        self.holding_duration[agent] = new_duration + yellow_time + all_red_time + min_green_time
                    else:
                        self.time_since_last_phase_switch[agent] += 1
                        self.holding_duration[agent] += new_duration
                else:
