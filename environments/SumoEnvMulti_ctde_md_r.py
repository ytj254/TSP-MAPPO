"""This environment is based on SumoEnvMulti_ctde_input_action.py.
This environment is for multi-discrete action space.
This environment is used for the r_corridor scenario"""
import functools
...
class SumoEnvMulti(ParallelEnv):
    ...
    def __init__(self, sumo_cmd, map_name='r_corridor', obs_type='img', cv_only=cv):
        # To obey format rules
        self.active_agents = None
        self.time_since_last_phase_switch = None
        self.time_since_last_act = None
        self.holding_duration = None
    ...
    def reset(self, seed=None, options=None):
        ...
        self.time_since_last_act = {a: 0 for a in self.agents}
        self.holding_duration = {a: 0 for a in self.agents}
    ...
    def step(self, actions):
        ...
        # print(f'-------actions: {actions}, step: {self.sim_step}, active agent: {self.active_agents}--------')
        self.act_multi_action(actions)
        self.simulate()
        ...
        # print(f'--------------Time step: {self.sim_step}------------------')
        # print('Time since last act:         ', self.time_since_last_act)
        # print('Time since last phase change:', self.time_since_last_phase_switch)
        # print('Holding duration:            ', self.holding_duration)
        # print('Active agent:', self.active_agents)
        # Reward type 1
        rewards = {
        ...
        # Reward type 2
        # rewards = {
        #     a: ((self.last_tot_person_delays[a] - self.observe(a)[1]) / self.time_since_last_act[a]) * 0.001
        #     for a in self.agents if self.active_agents[a]
        # }
        # print(rewards)
            ...
            observation_space = spaces.Dict(
                {
                    ...
                    'phases': spaces.Dict(
                        {
                            a: spaces.Box(low=0, high=1, shape=(self.num_one_hot_phases,),
                                          dtype=np.float64)
                            for a in self.agents
                        }
                    ),
    ...
    def action_space(self, agent):
        return spaces.MultiDiscrete([len(self.sig_configs[agent]['phase_pairs']), 40])
    ...
    def get_phase_states(self, agent):
        ...
        for a, phase_state in phase_states.items():  # Green phase
            if phase_state in self.sig_configs[agent]['action_phase_map'].values():
                phase_num = [key for key, value in self.sig_configs[agent]['action_phase_map'].items()
                             if value == phase_state][0]
            elif 'y' in phase_state.lower():  # Yellow phase
                phase_num = self.num_one_hot_phases - 2
            else:  # Red phase
                phase_num = self.num_one_hot_phases - 1
            # One-hot encode the phase number
            one_hot_phase = np.zeros(self.num_one_hot_phases)
    ...
    def act_multi_action(self, actions):
        # print(self.time_since_last_phase_switch)
        # print(self.holding_duration)
        for agent in self.agents:
            # holding_duration = yellow_time + all_red_time + min_green_time + self.green_duration[agent]
            if self.time_since_last_phase_switch[agent] < self.holding_duration[agent]:
                self.active_agents[agent] = False
                self.time_since_last_phase_switch[agent] += 1
                self.time_since_last_act[agent] += 1
            ...
            # Agent should act now
                ...
                if agent in actions.keys():
                    self.active_agents[agent] = False
                    self.time_since_last_act[agent] = 0
                    new_phase, new_duration = actions[agent].tolist()
                    new_phase_state = self.sig_configs[agent]['action_phase_map'][new_phase]
                    ...
                    if new_phase_state != current_state:
                        ...
                        self.holding_duration[agent] = new_duration + 1 + yellow_time + all_red_time + min_green_time
                    else:
                        self.time_since_last_phase_switch[agent] += 1
                        self.holding_duration[agent] = self.time_since_last_phase_switch[agent] + new_duration + 1
                else:
                    self.active_agents[agent] = True
                    self.time_since_last_phase_switch[agent] += 1
                    self.time_since_last_act[agent] += 1
                    # print(f'The holding duration for {agent} is: {self.holding_duration[agent]}')
